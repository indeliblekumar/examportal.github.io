...
//  $errors = query_outcome_echo($query,$result,$errors);
  ...
  $hash = md5($str);
  setcookie('ac_id',$tid,false,'/'); // ac stands for admin cookie
  setcookie('ac_hash',$hash,false,'/');
  
  userdb_update_hash( $tid, $hash, $udb);
}
else
{
  if(isset($_COOKIE['ac_id']) && isset($_COOKIE['ac_hash']))
  {
    $tid = $_COOKIE['ac_id'];
    $ac_hash = $_COOKIE['ac_hash'];    
...
function get_user_pword_login($tries=0, $more_comments = "", $parent = "")
{
  $next_try = $tries + 1;
