...
//  $errors = query_outcome_echo($query,$result,$errors);
  ...
  if ($access != "P") // only principals can access this directory
  {
    mysql_close($mylink);
    $comment = "Bad user name or password. You must have principal 
                access to log in.";
    get_user_pword_login($tries, $comment, $parent);
    exit();
  }
  ...
  $hash = md5($str);
  setcookie('pc_id',$tid,false,'/');  // pc stands for principal cookie
  setcookie('pc_hash',$hash,false,'/');
  
  userdb_update_hash( $tid, $hash, $udb);
}
else
{
  if(isset($_COOKIE['pc_id']) && isset($_COOKIE['pc_hash']))
  {
    $tid = $_COOKIE['pc_id'];
    $pc_hash = $_COOKIE['pc_hash'];    
                                 ...
                                 $parent = "")
{
  ...
  if($tries == 1)
    $comments = "<h2>Welcome to principal administration.</h2>
                 <h3>$more_comments</h3>
                 <h3>Please log in.</h3>";
  else
    $comments="<h2>Principal database administration.</h2>
           <h3>$more_comments</h3>
           <h3>Please log in.</h3>";
