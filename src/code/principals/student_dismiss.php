...
function get_dismiss_form($student_arr, $dismissed)
{
  $id = $student_arr['id'];
  ...
  $dismissing_str = $dismiss_str . "ing";
...
{
  echo startTable();
  $message = "The student is now dismissed";
...
function show_student_info_table($student_arr,$dismissed)
{
  $hr_num = $student_arr['hr'];
  show_table_column_title($hr_num);
  $edit_form = get_dismiss_form($student_arr,$dismissed);
...
function insert_dismiss ($id, $gone, $date, $time, $reason)
{
  $query = "insert into dismiss(id,gone,date,time,reason)
            values($id,'$gone','$date','$time','$reason')";
  $result = mysql_query($query);
...
var str='You will make the student dismiss. Educators will
...
$dismissed = is_dismissed($id,$date);
...
if(!empty($dismiss))
{
  if($dismissed)
  {
    ...
    $result = insert_dismiss ($id, "0", $date, $time, $reason);
    if($result === FALSE) $err++;
    else $dismissed = 0;
    ...
    if($err)
      ...
      $comments="The student has now been undismissed.<br>
                 In other words, the student should have returned to school
  ...
  else
  {
    $err = 0;
    $reason = addslashes($reason);
    $result = insert_dismiss ($id, "1", $date, $time, $reason);
    if($result === FALSE) $err++;
    else $dismissed = 1;
    ...
    $comments="The student is currently dismissed.<br>
               In other words, the student has been excused from school for the day.";
  }
}
else
{
  if($dismissed)
    ...
    $comments="The student is not dismissed.<br>
          In other words, the student has not been excused from school for the day.";
}
title_comment_div_noend($page_title, $comments);  
show_student_info_table ($stud_r,$dismissed);
